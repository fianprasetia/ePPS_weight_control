...
controller.selectGoodsIssue = async function (req, res) {
    const dataGoodsIssue = req.body["dataGoodsIssuePdf"][0]["goods_issue_POST"];
    const dataSignature = req.body["dataGoodsIssuePdf"][0]["signature_POST"];
    ...
    const language = req.body["dataGoodsIssuePdf"][0]["language_POST"];
    const messages = translations.find(item => item.language === language);

    const order = dataGoodsIssue
    const imageEmployeeWarehouse = dataGoodsIssue["employeeSignature"]["photo"]
    const code_goods_issue = order["code_goods_issue"];
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', 'attachment; filename="goods_issue.pdf"');
    ...
    const safeFileName = code_goods_issue.replace(/\//g, '_'); // Mengganti '/' dengan '_'
    const outputPath = path.join(__dirname, '../public/file/issue/', `${safeFileName}.pdf`);
    ...
    // Header
    const title = messages.content.goods_issue_slip.toUpperCase();
    doc.fontSize(20).font('Helvetica-Bold').fillColor('#0665d0').text(title, { align: 'center' });
    doc.fontSize(10).font('Helvetica').fillColor('black').text(`#${order.code_goods_issue.toUpperCase()}`, { align: 'center' });
    doc.moveDown(1);
    ...
    doc.moveDown(2);
    const lineY = starttop + 30; // Posisi garis di bawah teks 55
    doc.strokeColor(bgColor).strokeOpacity(0.3).moveTo(30, lineY).lineTo(563, lineY).stroke();
    let texttop = starttop + 45
    doc.fontSize(9).text(messages.content.text_gi.replace("{{date}}", eppsLib.ddmmyyyy(order.date)).replace("{{warehouse}}", order.warehouse).replace("{{request}}", order.employeeRequest.fullname).toUpperCase(), marginleft, texttop);

    let headertop = starttop + 60;
    let rowHeight = 20;
    let textColor = 'white';
    doc.fontSize(10);
    ...
    doc.text(order.log_goods_issue_approvals[0].hrd_employee.fullname.toUpperCase(), 381, approveTOC + 130, { width: 182, align: 'center' })
