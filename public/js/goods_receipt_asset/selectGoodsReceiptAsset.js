...
async function selectContent() {
 var token = await JSON.parse(getCookie("dataToken"));
  ...
  async function dataContent(data) {
    ...
    document.getElementById("titleContent").innerHTML = content["goods_receipt_asset"];
    document.getElementById("titlePage").innerHTML = content["goods_receipt_asset"];
    document.getElementById("titleContent").innerHTML = content["goods_receipt_asset"];
    ...
    document.getElementById("noGRListData").innerHTML = content["goods_receipt_number"];
    document.getElementById("noPOListData").innerHTML = content["pr_number"];
        ...
        for (i = 0; i < responseData[0]["details"].length; i++) {
          ...
          tableItem +=
            "<tr>\
                  ...
                  <td class='fw-light text-center'><input type='text' onkeyup='typeformatRupiah; compareQty()' class='form-control text-end' id='qtyReceivedDataBody' value='0' onfocus='removeZero(this)' name='qtyReceivedDataBody[]'  style='width: 150px;'></td>\
...
async function SelectGoodsReceipt(startDateSearch, endDateSeacrh, codeWareHouse) {
 ...
 var token = await JSON.parse(getCookie("dataToken"));
  ...
  var xhr = new XMLHttpRequest();
  var url = mainUrl + "goodsreceiptasset";
  xhr.onerror = function () {
    Dashmix.helpers("jq-notify", {
      type: "danger",
      z_index: 2000,
      ...
      if (response["access"] == "success") {
        ...
        for (i = 0; i < responseData.length; i++) {
          ...
          if (statusTemp == 0) {
            ...
            deleteData = "<button title='delete' type='button' id='" + noGR + "' onclick='showModalDeletePostingGoodsReceipt(id)' class='btn btn-danger'><i class='fa-regular fa-trash-can'></i></button>";
            approvalData = "";
            viewData = "";
            pdfGR = "";
            status = request;
...
function checkDate() {
  ...
  let startDate = new Date(startDatePeriod); 
      ...
      if (response["access"] == "success") {
        ...
        dataApproval.forEach((data, index) => {
          ...
          if (data.date == null) {
            ...
            const dateApprove = new Date(data.date); 
            const dd = String(dateApprove.getDate()).padStart(2, "0"); 
            const mm = String(dateApprove.getMonth() + 1).padStart(2, "0"); 
            const yyyy = dateApprove.getFullYear(); 
            const HH = String(dateApprove.getHours()).padStart(2, "0"); 
            const MM = String(dateApprove.getMinutes()).padStart(2, "0"); 
