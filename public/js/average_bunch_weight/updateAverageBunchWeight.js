async function showUpdateAverageBunchRate(code) {
    const period = code.getAttribute('period')
    const division = code.getAttribute('division');
   ...
   if (!token) {
    ...
    var url = mainUrl + "averagebunchrate/bycode"
    ...
    xhr.onloadend = async function () {
        if (this.readyState == 4 && this.status == 200) {
            ...
            if (response["access"] == "success") {
                ...
                let estateNow = responseData["periodNow"][0]["estate"]
                let divisionNow = responseData["periodNow"][0]["division"]
                selectDivision(estateNow, divisionNow)
                selectEstate(estateNow)
                document.getElementById("periodDateHeader").value = mmyyyy(responseData["periodNow"][0]["period"])
                const uniquePeriodNow = responseData.periodNow.filter(
                    (value, index, self) =>
                        index === self.findIndex((t) => t.block === value.block)
                );

                uniquePeriodNow.forEach((now, index) => {
                    const avg = responseData.periodBefore.find(
                        (a) => a.block === now.block
                    );
                    addRow(
                        index + 1,
                        now.block,
                        avg ? avg.average_bunch_rate : 0,
                        now.average_bunch_rate
                    );
                });
                document.getElementById("load").innerHTML = "<a id='doneBtn' type='submit' onclick='updateAverageBunchRate()' class='btn  btn-primary'>" + kapital(process) + "</a>"
...
async function updateAverageBunchRate() {
   ...
   if (!token) {
    ...
    var xhr = new XMLHttpRequest();
    var url = mainUrl + "averagebunchrate/update";
