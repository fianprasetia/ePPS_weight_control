...
async function selectContent() {
   ...
   if (!token) {
    ...
    async function dataContent(data) {
        ...
        document.getElementById("titlePage").innerHTML = filterLanguage[0]["content"]["department"]
        document.getElementById("titleModal").innerHTML = filterLanguage[0]["content"]["department"]
                ...
                for (i = 0; i < responseData.length; i++) {
                    tableItem +=
                        "<tr>\
                            ...
                            <td class='fw-light text-uppercase'>" + responseData[i]["hrd_department_translations"][0]["translation"] + "</td>\
                            <td class='fw-light text-center text-uppercase'>" + (responseData[i]["status"] === 0 ? nonactive : active) + "</td>\
                            <td class='fw-light text-center'><div class='btn-group'><button type='button' id ='" + responseData[i]["department_code"] + "' onclick='showModalUpdateDepartment(id)' class='btn btn-primary'><i class='fa-regular fa-pen-to-square'></i></div></td>\
                        </tr>";
                    no++;
                }
