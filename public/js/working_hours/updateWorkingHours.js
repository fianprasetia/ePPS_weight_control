async function showModalUpadateWorkHours(id) {
    ...
    var url = mainUrl + "workinghours/id"
    ...
    xhr.onloadend = async function () {
        if (this.readyState == 4 && this.status == 200) {
            ...
            if (response["access"] == "success") {
                ...
                document.getElementById("idWorkHours").value = responseData[0]["id_working_hours"]
                // document.getElementById("name").value = responseData[0]["name"]
                document.getElementById("onDuty").value = formatTime(responseData[0]["on_duty_time"])
                document.getElementById("offDuty").value = formatTime(responseData[0]["off_duty_time"])
                document.getElementById("lateTolerance").value = formatTime(responseData[0]["late_time"])
                document.getElementById("earlyTolerance").value = formatTime(responseData[0]["leave_early_time"])
                document.getElementById("checkInOpen").value = formatTime(responseData[0]["beginning_in"])
                document.getElementById("checkInClosed").value = formatTime(responseData[0]["ending_in"])
                document.getElementById("checkOutOpen").value = formatTime(responseData[0]["beginning_out"])
                document.getElementById("checkOutClosed").value = formatTime(responseData[0]["ending_out"])
                var languageData = await responseData[0]["hrd_working_hours_translations"]
                for (i = 0; i < languageData.length; i++) {
                    document.getElementById("language" + i).value = await responseData[0]["hrd_working_hours_translations"][i]["translation"]
                }
                document.getElementById("load").innerHTML = "<a id='cancelBtn' onclick='closeModal()' class='btn  btn-danger'>" + kapital(cancel) + "</a> <a id='doneBtn' type='submit' onclick='updateWorkingHours()' class='btn  btn-primary'>" + kapital(done) + "</a>"
...
async function updateWorkingHours() {
   ...
   if (!token) {
    ...
    const id = document.getElementById("idWorkHours").value
    ...
    if (valuesLanguage == "" || onDuty == "" || offDuty == "" || lateTolerance == "" || earlyTolerance == "" || checkInOpen == "" || checkInClosed == "" || checkOutOpen == "" || checkOutClosed == "") {
        return false
    }
    var xhr = new XMLHttpRequest();
    var url = mainUrl + "workinghours/update/" + id;
