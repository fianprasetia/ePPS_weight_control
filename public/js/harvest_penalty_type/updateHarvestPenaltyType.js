async function showModalUpdateHarvestPenaltyType(id) {
    ...
    var url = mainUrl + "harvestpenaltytype/bycode"
    ...
    xhr.onloadend = async function () {
        if (this.readyState == 4 && this.status == 200) {
            ...
            if (response["access"] == "success") {
                ...
                document.getElementById("code").value = responseData[0]["code_harvest_penalty"]
                ...
                document.getElementById("load").innerHTML = "<a id='cancelBtn' onclick='closeModal()' class='btn  btn-danger'>" + kapital(cancel) + "</a> <a id='doneBtn' type='submit' onclick='UpdateHarvestPenaltyType()' class='btn  btn-primary'>" + kapital(done) + "</a>"
...
async function UpdateHarvestPenaltyType() {
   ...
   if (!token) {
    ...
    const codelMenu = JSON.parse('{"code_harvest_penalty_POST":"' + code + '"}');
    $.extend(codelMenu, usernameMenu, languageMenu, { detail });
    dataHarvestPenaltyType.push(codelMenu);
    var xhr = new XMLHttpRequest();
    var url = mainUrl + "harvestpenaltytype/update";
