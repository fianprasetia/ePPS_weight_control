...
async function selectContent() {
   ...
   if (!token) {
    ...
    async function dataContent(data) {
        ...
        document.getElementById("titlePage").innerHTML = content["warehouse_value"];
        ...
        document.getElementById("qtyBeginning").innerHTML = content["qty"]
        document.getElementById("unitPriceBeginning").innerHTML = content["unit_price"]
        document.getElementById("totalPriceBeginning").innerHTML = content["total_price"]
        document.getElementById("qtyIncomming").innerHTML = content["qty"]
        document.getElementById("unitPriceIncomming").innerHTML = content["unit_price"]
        document.getElementById("totalPriceIncomming").innerHTML = content["total_price"]
        document.getElementById("qtyOutgoing").innerHTML = content["qty"]
        document.getElementById("unitPriceOutgoing").innerHTML = content["unit_price"]
        document.getElementById("totalPriceOutgoing").innerHTML = content["total_price"]
        document.getElementById("qtyEnding").innerHTML = content["qty"]
        document.getElementById("unitPriceEnding").innerHTML = content["unit_price"]
        document.getElementById("totalPriceEnding").innerHTML = content["total_price"]
    ...
    xhr.onloadend = async function () {
        if (this.readyState == 4 && this.status == 200) {
            ...
            if (response["access"] == "success") {
                ...
                setTimeout(function () {
                    window.location.href = "/warehouse_inventory_value_report";
                }, 3000);
            }
        } if (this.status == 404) {
...
async function selectWarehouseItem() {
    ...
    try {
        ...
        if (result.access === "success") {
            ...
            for (let i = 0; i < responseData.length; i++) {
                ...
                tableItem += `
                    <tr>
                        ...
                        <td class='fw-light text-uppercase text-end'>${formatRupiah(item.beginning_price)}</td>
                        <td class='fw-light text-uppercase text-end'>${formatRupiah(beginningTotalPrice)}</td>
                        <td class='fw-light text-uppercase text-center'>${item.incoming_qty}</td>
                        <td class='fw-light text-uppercase text-end'>${formatRupiah(item.incoming_price)}</td>
                        <td class='fw-light text-uppercase text-end'>${formatRupiah(incomingTotalPrice)}</td>
                        <td class='fw-light text-uppercase text-center'>${item.outgoing_qty}</td>
                        <td class='fw-light text-uppercase text-end'>${formatRupiah(item.outgoing_price)}</td>
                        <td class='fw-light text-uppercase text-end'>${formatRupiah(outgoingTotalPrice)}</td>
                        <td class='fw-light text-uppercase text-center'>${endingQty}</td>
                        <td class='fw-light text-uppercase text-end'>${formatRupiah((isNaN(endingPrice) ? 0 : endingPrice))}</td>
                        <td class='fw-light text-uppercase text-end'>${formatRupiah(endingTotalPrice)}</td>
                    </tr>`;
                no++
            }
            ...
            let tableItemFoot = `
                    <tr>
                      <td id="subtotalFoot" class="text-center text-uppercase" colspan="5">${total}</td>
                      <td id="totalOpen" class="text-end text-uppercase" colspan="3">${formatRupiah(totalBeginning)}</td>
                      <td id="totalOpen" class="text-end text-uppercase" colspan="3">${formatRupiah(totalIncoming)}</td>
                      <td id="totalDebit" class="text-end text-uppercase" colspan="3">${formatRupiah(totalOutgoing)}</td>
                      <td id="totalCredit" class="text-end text-uppercase" colspan="3">${formatRupiah(totalEnding)}</td>
                    </tr>`
...
async function selectWarehouseInventoryValueExcel() {
    ...
    try {
        ...
        if (result.access === "success") {
            ...
            await warehouseInventoryValueToExcel(responseData)
        } else if (result.access === "failed") {
            Dashmix.helpers("jq-notify", {
                type: "danger",
                z_index: 2000,
...
async function warehouseInventoryValueToExcel(responseData) {
    const language = await JSON.parse(getCookie("language"));
   ...
   if (!token) {
    ...
    const loadData = {
        language_POST: language,
        warehouse_inventory_value_POST: responseData
    };
    ...
    const url = secondUrl + "warehouseinventoryvalue";
    xhr.onerror = function () {
        Dashmix.helpers("jq-notify", {
            z_index: 2000,
            type: "danger",
        ...
        if (this.readyState == 4 && this.status == 200) {
            ...
            if (response["access"] === "success") {
                ...
                setTimeout(() => {
                    ...
                    document.getElementById("doc").innerHTML = "<button onclick='selectWarehouseInventoryValueExcel()' class='btn' href='#'><img class='' style='width:35px;' src='file/icon/xls.png'></button>"
                }, 3000);
            } else {
                Dashmix.helpers("jq-notify", {
                    z_index: 2000,
