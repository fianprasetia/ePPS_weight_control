async function showUpdateBasicSalary(code) {
    const period = code.getAttribute('period')
    const location = code.getAttribute('location');
    const type = code.getAttribute('employeeType');
   ...
   if (!token) {
    ...
    var url = mainUrl + "basicsalary/code"
    ...
    xhr.onloadend = async function () {
        if (this.readyState == 4 && this.status == 200) {
            ...
            if (response["access"] == "success") {
                ...
                var dataEmployee = responseData["merged"]
                selectLocation(responseData["header"]["location"])
                selectEmployeeType(responseData["header"]["type"])
                dataEmployee.forEach((emp, index) => {
                    addRow(
                ...
                document.getElementById("periodDateHeader").value = responseData["header"]["period"]
                document.getElementById("load").innerHTML = "<a id='doneBtn' type='submit' onclick='updateBasicSalary()' class='btn  btn-primary'>" + kapital(process) + "</a>"
...
async function updateBasicSalary() {
   ...
   if (!token) {
    ...
    var xhr = new XMLHttpRequest();
    var url = mainUrl + "basicsalary/update ";
