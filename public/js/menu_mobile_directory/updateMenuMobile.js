async function showModalUpdateMenu(id) {
    ...
    var url = mainUrl + "menumobile/code"
    ...
    xhr.onloadend = async function () {
        if (this.readyState == 4 && this.status == 200) {
            ...
            if (response["access"] == "success") {
                ...
                var languageData = await responseData[0]["adm_menu_mobile_translations"]
                for (i = 0; i < languageData.length; i++) {
                    document.getElementById("language" + i).value = await responseData[0]["adm_menu_mobile_translations"][i]["translation"]
                }
...
async function updateMenu() {
   ...
   if (!token) {
    ...
    var xhr = new XMLHttpRequest();
    var url = mainUrl + "menumobile/update/" + code;
