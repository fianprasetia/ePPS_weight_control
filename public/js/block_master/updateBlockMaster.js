async function showUpdateBlockMaster(code) {
   ...
   if (!token) {
    ...
    var url = mainUrl + "blockmaster/bycode"
    ...
    xhr.onloadend = async function () {
        if (this.readyState == 4 && this.status == 200) {
            ...
            if (response["access"] == "success") {
                ...
                await Promise.all([
                    selectBlock(responseData[0]["code_company"]),
                    selectTreeClass(responseData[0]["tree_class"]),
                    selectActivityType(responseData[0]["block_status"]),
                    selectSoilCode(responseData[0]["soil_code"]),
                    selectSoilClassification(responseData[0]["soil_classification"]),
                    selectTopologi(responseData[0]["topography"]),
                    selectNucleusPlasma(responseData[0]["nucleus_plasma"]),
                    selectSeedType(responseData[0]["seed_type"]),
                ]);
                document.getElementById("plantingYearHeader").value = responseData[0]["planting_year"] || "";
                document.getElementById("plantedAreaHeader").value = responseData[0]["planted_area"];
                document.getElementById("unplantedAreaHeader").value = responseData[0]["unplanted_area"];
                document.getElementById("numberOfTreesHeader").value = responseData[0]["number_of_trees"] || "";
                document.getElementById("startHarvestHeader").value = mmyyyy(responseData[0]["start_harvest"]) || "";
                document.getElementById("load").innerHTML = "<a id='doneBtn' type='submit' code='" + code + "' onclick='updateBlockMaster(this)' class='btn  btn-primary'>" + kapital(done) + "</a>"
...
async function updateBlockMaster() {
   ...
   if (!token) {
    ...
    var xhr = new XMLHttpRequest();
    var url = mainUrl + "blockmaster/update";
