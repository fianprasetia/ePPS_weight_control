async function selectContent() {
 var token = await JSON.parse(getCookie("dataToken"));
  ...
  async function dataContent(data) {
    ...
    // Assign variabel dengan data konten
    all = content["all"];
    add = content["add"];
    overload = content["overload"];
    ...
    status_500 = content["status_500"];
    status_404 = content["status_404"];
    status_401 = content["status_401"];
    active = content["active"];
    nonactive = content["nonactive"];
    notice_posting = content["notice_posting"];
    notice_delete = content["notice_delete"];
    notice_date_request = content["notice_date_request"];
    request = content["request"];
    waiting_approval = content["waiting_approval"];
    approve = content["approve"];
    finding_suppliers = content["finding_suppliers"];
    reject = content["reject"];
    minlengthchar = content["minlengthchar"];
    no_item = content["no_item"];

    document.getElementById("purchaseRequestCapexPage").innerHTML = content["purchase_request_capex"];
    document.getElementById("titlePurchaseRequestCapexApproval").innerHTML = content["purchase_request_capex"];
    document.getElementById("titleContent").innerHTML = content["purchase_request_capex"];
    document.getElementById("titleItemMaster").innerHTML = content["purchase_request_capex"];
    // document.getElementById("prform").innerHTML = content["purchase_request_form"];
...
async function SelectPurchaseRequest() {
  ...
  document.getElementById("load").innerHTML = "<a id='cancelBtn' onclick='cancelTransaction()' class='btn  btn-danger'>" + kapital(cancel) + "</a> <a id='doneBtn' type='submit' onclick='InsertPurchaseRequest()' class='btn  btn-primary'>" + kapital(done) + "</a>";
  ...
  xhr.onloadend = async function () {
    if (this.readyState == 4 && this.status == 200) {
      ...
      if (response["access"] == "success") {
        ...
        var filterSubData = responseData.filter((filterSubData) => filterSubData.type == "CAPEX");
        var tableItem = "";
        var no = 1;
